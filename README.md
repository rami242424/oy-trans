...
- **14개 언어 정형 문구 99개** — 결제 / 택스리펀 / 교환·수하물 / 재고 / 추천 / 기타 6개 카테고리
- **고객 표시 화면** — 문구를 탭하면 라임 그린 전체 화면에 크게 표시
- **문구 체인** — 질문 문구에서 고객이 답을 선택하면 다음 문구로 자동 연결 (최대 4단계, 11개 연결)
...
├── data/
│   ├── phrases.json         # 문구 99개 × 14개 언어 + 체인 정보
...
체인을 붙이는 기준은 **"고객의 답을 크루가 알아들을 수 없는 질문인가"** 입니다. "카드로 하시겠어요?"는 고객이 카드를 꺼내면 답이 보이므로 제외했고, 99개 중 11개에만 적용했습니다.

실사용하면서 체인의 역할이 한 단계 넓어졌습니다. 처음에는 "질문 → 정보 안내"에서 끝났는데, 쇼핑백 흐름에서 **고객이 고른 뒤에도 확인이 필요하다**는 것을 발견했습니다. 가격 두 가지를 보여주면 고객이 손가락으로 가리키거나 고개를 끄덕이는데, 그게 어느 쪽인지 확실하지 않은 경우가 있었습니다.
...
고객이 직접 버튼을 누르면 **선택이 명확해지고, 확정 문구가 곧 영수증 전 최종 확인**이 됩니다. 체인이 "정보 전달"에서 "선택 확정"까지 담당하게 된 사례입니다.
