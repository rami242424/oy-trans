...
> ⚠️ 올리브영 공식 서비스가 아닌 개인 제작물입니다.

---

## 💡 왜 만들었나

인천공항 올리브영에는 하루에도 수많은 국적의 고객이 방문합니다.
크루가 익힌 기본 응대만으로는 부족한 상황이 자주 발생했습니다. 특히 **택스리펀과 기내반입 규정** 안내는 문장이 길고 복잡해서 영어로도 설명이 어려웠습니다.

기존 방식은 두 가지였습니다.

| 방법                             | 문제                                     |
| -------------------------------- | ---------------------------------------- |
| 해당 언어가 가능한 크루를 찾는다 | 매번 다른 크루의 업무를 중단시킴         |
| 번역 앱을 켜서 입력한다          | 매번 타이핑, 번역 부정확, 응대 속도 저하 |

두 방식 모두 비효율적이라, **매장에서 배운 MOT(Moment of Truth) 응대 기준에 맞춰 정형 문구를 미리 준비해두고 화면으로 보여주는** 앱을 기획하고 개발했습니다.
...
| 언어 선택             | 문구 목록             | 고객 화면                |
| --------------------- | --------------------- | ------------------------ |
| (screenshot_lang.png) | (screenshot_home.png) | (screenshot_display.png) |

| 문구 체인              | 자유 번역                  | 매장 지도            |
| ---------------------- | -------------------------- | -------------------- |
| (screenshot_chain.png) | (screenshot_translate.png) | (screenshot_map.png) |
...
AI 번역만 믿었다면 어색한 문구를 그대로 사용했을 것입니다. 나머지 언어(우즈베크어·몽골어·터키어)도 검수 대상으로 남겨두고 있습니다.

---

## 🎨 디자인

올리브영 2025 리브랜딩 톤을 참고하되, 공식 서비스로 오인되지 않도록 **OY-trans**라는 자체 워드마크를 사용했습니다.

- **흰 배경 + 굵은 검정 타이포** — 커머스 앱의 정보 밀도
- **라임 그린 `#8ED320`** — 고객 표시 화면 배경, 주요 액션 버튼
- **Pretendard Variable** — 한국어·중국어·일본어를 한 서체로 처리
- 라임 도트(`•`) — 올리브영 심볼 자리를 대체하는 아이덴티티 요소

고객 표시 화면은 **라임 배경 + 흰색 초굵은 번역문**으로, 매장 조명 아래에서도 멀리서 읽히도록 설계했습니다.

---

## 📊 실사용 피드백

배포 첫날 매장에서 사용하며 확인한 내용입니다.

- **피크타임에는 앱을 꺼내는 것 자체가 비용이다** → 최근 사용 문구·즐겨찾기 기능 추가의 근거
- **만든 사람은 편향돼 있다** — 개발자는 문구 위치를 다 알아 불편함을 못 느꼈지만 다른 크루는 찾지 못함 → 동료 테스트 필요성 확인
- **다국어가 가능한 크루에게도 효용이 있다** — "말하는 것보다 화면 보여주는 게 덜 지친다"
